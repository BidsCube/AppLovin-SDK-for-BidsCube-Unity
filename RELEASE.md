...
| Item | Format | Example (1.0.6) |
|------|--------|-----------------|
| **UPM `package.json` → `version`** | **`MAJOR.MINOR.PATCH`** only (Unity rejects `1.0.3.1`-style fourth segment). Optional prerelease: `1.0.6-rc.1`. | `1.0.6` |
| **Git tag** | `v` + the same version string | `v1.0.6` |
...
| **`Constants.SdkVersion`** | Unity package / user-agent string | Must match UPM `version`; **`Constants.NativeAndroidBidscubeSdkVersion`** pins the **default** Gradle line **`com.bidscube:bidscube-sdk`** as **`…@aar`** (e.g. **1.2.2**); optional **`BidscubeAndroidGradlePostprocessor.CoreDependencyMode`** overrides how core is injected |
...
```bash
git tag -a "v1.0.6" -m "com.bidscube.sdk 1.0.6"
git push origin "v1.0.6"
```

4. The **Release (GitHub)** workflow (`.github/workflows/release.yml`) validates `package.json`, builds a ZIP without `.git`, and creates a GitHub Release with **`Bidscube-SDK-Unity-{version}.zip`** (e.g. **`Bidscube-SDK-Unity-1.0.6.zip`**).
...
- **`release.yml`**:
  1. **Via tag:** after `git push origin v1.0.6` (tag `v*`). The value in `package.json` must match the tag without `v`.
