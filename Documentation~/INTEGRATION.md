...
- On Gradle export, **`BidscubeAndroidGradlePostprocessor`** injects AppLovin 13.x, Media3, IMA, UMP, Glide, Material, **and** the core Bidscube **`@aar`** coordinate into **`unityLibrary`**. By default **`NoDesugarMode = true`**: it does **not** inject **`coreLibraryDesugaring`** / **`desugar_jdk_libs`** / **`coreLibraryDesugaringEnabled`** — add those in **your** Gradle if **`CheckAarMetadata`** or Java 8+ APIs require desugaring; see **`ANDROID_BUNDLED_SDK.md`**. Set **`NoDesugarMode = false`** only if you want legacy plugin-managed desugaring on fresh exports when Gradle does not already declare it. Install Unity **Android Build Support** so the Editor script compiles. First Gradle resolve needs Maven Central access for **`com.bidscube:bidscube-sdk`**. Validate with clean cache, **`--refresh-dependencies`**, full **`assemble*`**, and a **lower-API** device smoke test.
