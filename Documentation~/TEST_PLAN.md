...
**Version alignment:** this adapter UPM declares **`com.bidscube.applovin.max` 1.0.14**, peer **`com.bidscube.sdk` 1.2.5**, native Android core **1.2.3**, MAX adapter / iOS pod **1.0.4** — see [`ANDROID_BUNDLED_SDK.md`](ANDROID_BUNDLED_SDK.md) and [`APPLOVIN_MAX.md`](APPLOVIN_MAX.md). Steps below assume **Gradle** checks from your project (**`BidscubeAndroidGradlePostprocessor`** in this package and/or **`com.bidscube.sdk`** or **manual** `unityLibrary` edits). Default Android mode is **`FullWithVideo`**; use **`LiteNoVideo`** only when validating the slim dependency graph.
