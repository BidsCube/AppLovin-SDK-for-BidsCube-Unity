...
namespace BidscubeSDK
{
    ...
    /// </summary>
    internal class AdViewController : MonoBehaviour
    {
...
#pragma warning restore CS0414
        ...
        public void Initialize(string placementId, AdType adType, IAdCallback callback = null, AdPosition position = AdPosition.Unknown)
        {
            ...
            // Start with Unknown position - will be set when adm response is received
            // Video ads are always full screen, but we'll set that in MarkAdAsLoaded()
            if (adType == AdType.Video)
            {
                // Video ads are always full screen, but we'll set it properly in MarkAdAsLoaded()
                _currentPosition = AdPosition.Unknown; // Will be set to FullScreen when ad loads
                Logger.Info("[AdViewController] Video ad detected - will be set to full screen when ad loads");
            }
            else
            {
        ...
        public void MarkAdAsLoaded()
        {
            ...
            // Position priority: Manual override (if toggled) > Server response > Unknown
            // When adm response is received, determine position based on:
            // 1. If manual position toggle is ON -> use manual position from dropdown
            // 2. Otherwise -> use position from server response (adm)
            // 3. If neither available -> use Unknown (centered)
            ...
            {
                // Manual position toggle is ON - use manual position (overrides server response)
                positionToUse = manualPosition;
                Logger.Info($"[AdViewController] Manual position toggle is ON - using MANUAL position: {manualPosition} (overrides server response: {serverPosition})");
            }
            else if (serverPosition != AdPosition.Unknown)
            {
                // Manual position toggle is OFF - use server response position from adm
                positionToUse = serverPosition;
                Logger.Info($"[AdViewController] Manual position toggle is OFF - using SERVER RESPONSE position from adm: {serverPosition} (value: {(int)serverPosition})");
            }
            else
            {
                // No position from server or manual override - keep Unknown (will be centered)
                positionToUse = AdPosition.Unknown;
                Logger.Info("[AdViewController] No position from server response or manual override - using Unknown (centered)");
            }

            // Set and apply the determined position
            _currentPosition = positionToUse;
            Logger.Info($"[AdViewController] Setting _currentPosition to: {positionToUse} (value: {(int)positionToUse})");
